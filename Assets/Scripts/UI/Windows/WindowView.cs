...
namespace HackingProject.UI.Windows
{
    public sealed class WindowView
    {
        ...
        public WindowView(VisualElement root, VisualElement titleBar, Label titleLabel, Button closeButton, VisualElement contentRoot)
        {
        ...
        public static WindowView Create(VisualTreeAsset template)
        {
            ...
            if (titleBar == null || titleLabel == null || closeButton == null || content == null)
            {
                throw new InvalidOperationException("[WindowView] Window template is missing required elements.");
            }

            return new WindowView(root, titleBar, titleLabel, closeButton, content);
        }
